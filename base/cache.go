...
func ConfigLoader(
    ...
    golog.Infof("Load %s Config Cache:(%s) %+v", name, key, config)
...
func TokenLoader(
    ...
    if nil != err {
        golog.Debugf("Try to request %s Token:(%s)", name, key)
        ...
        if nil == err && count > 0 {
            ...
            golog.Infof("Request %s Token:(%s) %+v", name, key, token)
            return gokits.NewCacheItem(key, lifeSpan, token), nil
        }
        golog.Warnf("Give up request %s Token:(%s), wait for next cache Query", name, key)
        return nil, errors.New("Query " + name + " Token Later")
    }
    golog.Debugf("Query %s Token:(%s) %+v", name, key, queryDest)

    isExpired := time.Now().Unix() >= queryDest.GetExpireTime()
    isUpdated := "1" == queryDest.GetUpdated()
    if isExpired && isUpdated { // 已过期 && 是最新记录 -> 触发更新
        golog.Debugf("Try to request and update %s Token:(%s)", name, key)
        ...
        if nil == err && count > 0 {
            ...
            golog.Infof("Request and Update %s Token:(%s) %+v", name, key, token)
            return gokits.NewCacheItem(key, lifeSpan, token), nil
        }
        golog.Warnf("Give up request and update %s Token:(%s), use Query result Temporarily", name, key)
    }
    ...
    golog.Infof("Load %s Token Cache:(%s) %+v, cache %3.1f min", name, key, token, ls.Minutes())
...
func requestUpdaterStrict(
    ...
    response, err := requestor(key)
    if nil != err {
        golog.Warnf("Request %s Token Failed:(%s) %s", name, key, err.Error())
        return nil, err
    }
    ...
    if nil != err || count < 1 { // 记录入库失败, 则查询记录并返回
        ...
        effectiveSpan := time.Duration(queryDest.GetExpireTime()-time.Now().Unix()) * time.Second // in second
        if effectiveSpan <= expireCriticalSpan {
            return nil, errors.New(fmt.Sprintf("Query %s Token:(%s) expireTime Failed", name, key))
        }
