...
// GetString the number of items in the List.
func (node List) Len() int {
    return len(node)
}

// GetString the idx'th item from the List.
...
func (node Scalar) String() (string, error) { return string(node), nil }

func (node Scalar) Int() (int64, error) {
    str, err := node.String()
    if err != nil {
        return 0, err
    }
...
func (node Scalar) Bool() (bool, error) {
    str, err := node.String()
    if err != nil {
        return false, err
    }
