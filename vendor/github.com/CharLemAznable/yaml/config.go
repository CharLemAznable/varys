...
func ReadString(yamlconf string) (*File, error) {
...
// ConfigString reads a YAML configuration from a static string.  If an error is
// found, it will panic.  This is a utility function and is intended for use in
// initializers.
func ConfigString(yamlconf string) *File {
    f, err := ReadString(yamlconf)
    if err != nil {
        panic(err)
    }
...
func (f *File) Get(spec string) (Node, error) {
...
func ScalarOf(node Node, spec string) (Scalar, error) {
    sc, ok := node.(Scalar)
    if !ok {
...
func StringOf(node Node, spec string) (string, error) {
    sc, err := ScalarOf(node, spec)
    if err != nil {
        return "", err
    }
...
func IntOf(node Node, spec string) (int64, error) {
    sc, err := ScalarOf(node, spec)
    if err != nil {
        return 0, err
    }
...
func BoolOf(node Node, spec string) (bool, error) {
    sc, err := ScalarOf(node, spec)
    if err != nil {
        return false, err
    }
...
func ListOf(node Node, spec string) (List, error) {
    ls, ok := node.(List)
    if !ok {
...
func MapOf(node Node, spec string) (Map, error) {
    mp, ok := node.(Map)
    if !ok {
        return nil, &NodeTypeMismatch{
            Full:     spec,
...
// format as that expected by Child.  If the final node is not a Scalar, GetString
// will return an error.
func (f *File) GetScalar(spec string) (Scalar, error) {
    node, err := f.Get(spec)
    if err != nil {
        return "", err
    }

    return ScalarOf(node, spec)
}

func (f *File) GetString(spec string) (string, error) {
...
func (f *File) GetList(spec string) (List, error) {
    node, err := f.Get(spec)
    if err != nil {
        return nil, err
    }

    return ListOf(node, spec)
}

func (f *File) GetMap(spec string) (Map, error) {
    node, err := f.Get(spec)
    if err != nil {
        return nil, err
    }

    return MapOf(node, spec)
}
...
//   .mapkey   - GetString the key 'mapkey' of the Node, which must be a Map
